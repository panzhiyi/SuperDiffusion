function [rec, prec, fb] = DrawFbeta(SMAP, smapSuffix, GT, gtSuffix, targetIsFg, targetIsHigh)
...
ALLPRECISION = zeros(num, 60);
ALLRECALL = zeros(num, 60);
parfor k = 1:num
    ...
    [precision, recall] = CalFbeta(smapImg, gtImg, targetIsFg, targetIsHigh);
...
PRPath=strcat([SMAP,'/'],'Fbeta' ,smapSuffix(1:end-4), '.txt');
dlmwrite(PRPath,['Precision' smapSuffix(1:end-4) '=['],'delimiter','','newline','pc','-append');
dlmwrite(PRPath,prec,'delimiter','\x20','newline','pc','-append');
dlmwrite(PRPath,'];','delimiter','','newline','pc','-append');
dlmwrite(PRPath,['Recall' smapSuffix(1:end-4) '=['],'delimiter','','newline','pc','-append');
dlmwrite(PRPath,rec,'delimiter','\x20','newline','pc','-append');  
dlmwrite(PRPath,'];','delimiter','','newline','pc','-append');
dlmwrite(PRPath,['Fbeta' smapSuffix(1:end-4) '=['],'delimiter','','newline','pc','-append');
dlmwrite(PRPath,fb,'delimiter','\x20','newline','pc','-append');  
dlmwrite(PRPath,'];','delimiter','','newline','pc','-append');
