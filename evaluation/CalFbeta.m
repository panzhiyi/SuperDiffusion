function [precision, recall] = CalFbeta(smapImg, gtImg, targetIsFg, targetIsHigh)
% Code Author: Wangjiang Zhu
% Email: wangjiang88119@gmail.com
% Date: 3/24/2014
smapImg = smapImg(:,:,1);
if ~islogical(gtImg)
    gtImg = gtImg(:,:,1) > 128;
end
if any(size(smapImg) ~= size(gtImg))
    error('saliency map and ground truth mask have different size');
end

if ~targetIsFg
    gtImg = ~gtImg;
end

gtPxlNum = sum(gtImg(:));
if 0 == gtPxlNum
    error('no foreground region is labeled');
end

means=mean(mean(smapImg));
rmeans=round(0.1*means:0.1*means:6*means);
rmeans(rmeans>255)=255;
if isempty(rmeans)
    rmeans=21:80;
end

targetHist = histc(smapImg(gtImg), 0:255);
nontargetHist = histc(smapImg(~gtImg), 0:255);

if targetIsHigh
    targetHist = flipud(targetHist);
    nontargetHist = flipud(nontargetHist);
end
targetHist = cumsum( targetHist );
nontargetHist = cumsum( nontargetHist );

mtargetHist=targetHist(256-rmeans);
mnontargetHist=nontargetHist(256-rmeans);

precision = mtargetHist ./ (mtargetHist + mnontargetHist);
if any(isnan(precision))
    precision(isnan(precision)) = 0;
    %warning('there exists NAN in precision, this is because  your saliency map do not range from 0 to 255\n');
end
recall = mtargetHist / gtPxlNum;
if any(isnan(recall))
    recall(isnan(recall)) = 0;
    %warning('there exists NAN in recall, this is because  your saliency map do not range from 0 to 255\n');
end